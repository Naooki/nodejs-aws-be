...
export class ProductRepository {
  ...
  async create({ title, description, price, count }: any) {
    ...
    try {
      ...
      const productsQueryRes = await client.query(productsQuery, [
        title,
        description,
        price,
      ]);
      ...
      const stocksQueryRes = await client.query(stocksQuery, [
        product.id,
        count,
      ]);
      item = {
  ...
  async createMany(items: any[]) {
    ...
    try {
      ...
      const createPromises = items.map(item => this.createItemInTransaction(client, item));
  ...
  private async createItemInTransaction(
    client: Client,
    { title, description, price, count }: any
  ) {
    const productsQuery = `insert into products (title, description, price) values ($1, $2, $3) returning id, title, description, price`;
    const productsQueryRes = await client.query(productsQuery, [
      title,
